...
class AgentManager:
    ...
    def log_event(self, agent_name, content, highlight=False):
        ...
        icons = {"Navigator": "🧭", "Curator": "🔨", "Evaluator": "⚖️", "Visualizer": "🎨", "System": "⚙️", "Tool": "🛠️", "GitLab Duo": "🦊"}
    ...
    def run_mission(self, user_prompt, mode="Dev"):
        ...
        while attempts <= max_retries:
            ...
            # ---------------------------------------------------------
            # 🦊 NEW: GITLAB DUO AGENT INTEGRATION (HACKATHON)
            # ---------------------------------------------------------
            if mode in ["Dev", "Sentinel"]:
                try:
                    from .duo_agent_handler import route_to_gitlab_duo
                    self.log_event("GitLab Duo", "Analyzing code structure and security protocols...")
                    duo_response = route_to_gitlab_duo(f"{mode} Task", draft)
                    self.log_event("GitLab Duo", f"Feedback: {duo_response['feedback']}", highlight=True)
                except ImportError:
                    pass
            # ---------------------------------------------------------
        ...
        if final_output:
            if mode in ["Dev", "Sentinel"]:
                ...
                branding = (
                    ...
                    "# 📅 Copyright © 2026. Licensed under MIT License.\n"
                    "# ==========================================\n\n"
                )
